...
def get_dataset_baseline(ckpt_dir: Path, bands=None) -> dict:
    ...
    for mat_name, relative_path in stats["materials"].items():
        ...
        for bf in batch_files:
            ...
            inc_ang = data["inc_ang"].unsqueeze(1)
            geo = torch.cat([amps, phases, h, inc_ang], dim=1)
            all_geos.append(geo)
...
def main():
    ...
    results, stats = get_dataset_baseline(ckpt_dir, bands=bands)
    ...
    for mat_name, res in results.items():
        ...
        top_k_indices = torch.topk(avg_abs, args.top_k).indices
        
        fig, axes = plt.subplots(args.top_k, 4, figsize=(24, 5 * args.top_k), squeeze=False, layout="constrained")
        ...
        for i, idx in enumerate(top_k_indices):
            ...
            ax_p.set_title(f"Rank {i+1} (Avg Abs: {score:.3f}) - P-Pol", fontsize=14)
            ...
            ax_s.set_title(f"Rank {i+1} (Avg Abs: {score:.3f}) - S-Pol", fontsize=14)
            
            # Plot Profile
            cmap = plt.cm.viridis
            ax_g = axes[i, 2]
            ax_g.plot(r_grid, prof, "k-", lw=1.5)
            ax_g.fill_between(r_grid, 0, prof, color=cmap(0.7), alpha=0.5)
            ax_g.set_ylim(0, max(120, grating_height * 1.2))
            ax_g.set_xlim(0, 1000)
            ax_g.set_title(f"Profile ($h={h_nm:.1f}$nm, $\\theta={inc_ang:.1f}^\\circ$)", fontsize=14)
