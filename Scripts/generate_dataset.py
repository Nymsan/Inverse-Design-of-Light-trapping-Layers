...
def get_lhs_samples(num_samples):
    ...
    amps = 0 + 20 * sample[:, 3:8]       # 0 to 20 nm
...
def main():
    ...
    for batch_idx in range(num_batches):
        ...
        for i in tqdm(range(start_idx, end_idx), desc=f"Batch {batch_idx:04d}"):
            ...
            config_normal = RCWAConfig(
                inc_ang=1e-3 * np.pi/180, azi_ang=1e-3 * np.pi/180, 
                h=float(h), order_N=args.order_N, n_layers=args.num_layers,
                grating_period=args.grating_period, nx=args.nx, ny=1,
                add_reflector=True, reflector_type='pec', subpixel=True
            )
            A_film_norm, A_grat_norm = get_absorptance_curve(params_x=params_x, params_y=None, wavelengths=wavelengths, config=config_normal)
            
            # 2. Oblique Incidence Calculation
            inc_rad = (inc_ang_deg + 1e-3) * np.pi/180
            azi_rad = (azi_ang_deg + 1e-3) * np.pi/180
            config_oblique = RCWAConfig(
                inc_ang=inc_rad, azi_ang=azi_rad,
                h=float(h), order_N=args.order_N, n_layers=args.num_layers,
                grating_period=args.grating_period, nx=args.nx, ny=1,
                add_reflector=True, reflector_type='pec', subpixel=True
            )
            A_film_obl, A_grat_obl = get_absorptance_curve(params_x=params_x, params_y=None, wavelengths=wavelengths, config=config_oblique)
