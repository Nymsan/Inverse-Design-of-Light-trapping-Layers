...
Essentially, the original field $x$, which is a forward pass of a physics simulator, has to be solved for, after which the figure of merit dependence is often a scalar derived by a vector value, $\frac{\mathcal{L}}{x}$.
From here, the adjoint field can be simulated backwards by realizing that solving the system for the adjoint variable is essentially the problem in reverse. Mathematically, this is very close to how gradients flow backwards in reverse-mode auto differentiation software such as Pytorch, and 
...
For an example of the physics, see \cite{Hughes2018}, and for an example of 