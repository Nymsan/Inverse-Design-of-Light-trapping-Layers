...
Calling the number of layers $N_L$ and the number of wavelengths $N_w$, the total algorithmic time of RCWA is $N_L \cdot N_w \cdot O(N^6)$, which scales extremely poorly for 3 dimensions. I will discuss this a bit more in my results and numerical implementation, but compare for a moment to a good surrogate solver.
The target is the absorptance curve, no matter the input space. If the design space is 7 input amplitudes, 7 input phases, a height and an incident angle, this makes the first layers a bit denser in the neural networks, but all in all, it barely changes the weights, and therefore barely changes the inference time.
